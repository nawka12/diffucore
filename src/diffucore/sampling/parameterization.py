"""Model parameterization: a model's discrete training schedule in the
continuous sigma space samplers use.

:class:`DiscreteSchedule` builds ``sigma(t) = sqrt((1 - alphas_cumprod) /
alphas_cumprod)`` from ``betas`` (Ho et al., 2020) and converts sigma <-> t.
A :class:`Scaling` turns a prediction into ``x0 = c_skip * x + c_out *
model(c_in * x, t(sigma))``: :class:`EpsScaling` (eps-prediction),
:class:`VScaling` (v-prediction, Salimans & Ho 2022) with Karras et al. (2022)
preconditioning, and :class:`FlowMatchingConstScaling` for CONST rectified flow
(Anima, Flux, SD3).
"""
...
def make_betas(
    ...
    *,
    ...
    """Training beta schedule: ``scaled_linear`` (the SD/LDM default) or
...
def rescale_zero_terminal_snr(sigmas: torch.Tensor) -> torch.Tensor:
    """Rescale an ascending sigma table to (near-)zero terminal SNR (Lin et al.,
    2024), keeping ``sigma_min``; the terminal is clamped so ``sigma_max`` stays
    finite. Needs v-prediction.
    """
...
class DiscreteSchedule:
    """Per-timestep sigma table (ascending) from training betas, with sigma <-> t
    interpolated in ``log(sigma)``. ``zero_terminal_snr`` rescales it.
    """
...
class Scaling:
    """Base parameterization: :meth:`scalings` returns ``(c_skip, c_out, c_in)``.
    """
...
# sigma**2 in fp32: ZTSNR's sigma_max ~4500 overflows fp16 when squared.
...
class FlowMatchingConstScaling(Scaling):
    """CONST rectified flow (Anima / Flux / SD3): ``x_t = (1 − σ)·x0 + σ·ε``, the
    model predicts ``v = ε − x0``, so ``c_skip = 1``, ``c_out = −σ``, ``c_in = 1``.
    The σ-space samplers integrate it exactly since ``(x − x0)/σ = v``.
    """
