...
# ── REPRISE ───────────────────────────────────────────────────────────
# Restart sampling over the cogent3_pump core: cogent3_pump segments separated
# by exact forward-process jumps back up a high-σ band. The pins below are the
# degradation invariants — the ones that make it safe to ship a sampler whose
# whole value is what happens *between* its ODE solves.


def _reprise_sigmas(name, n, shift=3.0):
    return (S.flow_matching_schedule(n, shift=shift) if name == "flow"
            else S.flow_table_schedule(name, shift, n))


def _cogent3_pump(model, x_init, sigmas, seed):
    return _last_nonzero_latent(K.sample_cogent3, model, x_init, sigmas,
                                pump_strength=0.08, model_type="flow", shift=3.0,
                                generator=torch.Generator().manual_seed(seed))


def test_reprise_restarts_zero_is_cogent3_pump():
    # restarts=0 ⇒ no segments ⇒ the whole schedule goes to plain cogent3_pump,
    # bit-for-bit — which also pins the pump as on by default.
    torch.manual_seed(0)
    target = torch.randn(1, 4, 8, 8)
    model = _anneal_sigma_dependent_model(target)
    sigmas = S.flow_matching_schedule(16, shift=3.0)
    x_init = torch.randn(1, 4, 8, 8)
    got = _last_nonzero_latent(K.sample_reprise, model, x_init, sigmas, restarts=0,
                               model_type="flow", shift=3.0,
                               generator=torch.Generator().manual_seed(5))
    assert torch.equal(got, _cogent3_pump(model, x_init, sigmas, 5))
    no_pump = _last_nonzero_latent(K.sample_cogent3, model, x_init, sigmas,
                                   model_type="flow", shift=3.0,
                                   generator=torch.Generator().manual_seed(5))
    assert not torch.allclose(got, no_pump, atol=1e-5)


def test_reprise_band_off_grid_is_cogent3_pump():
    # A band the schedule has no point in (requested as 0.90–0.95 on a schedule
    # whose points step over it) must degrade the same way.
    torch.manual_seed(1)
    target = torch.randn(1, 4, 8, 8)
    model = _anneal_sigma_dependent_model(target)
    sigmas = S.flow_matching_schedule(8, shift=3.0)
    assert K._reprise_segments(sigmas, restart_lo=0.90, restart_hi=0.95,
                               restarts=3, draft_stride=2, model_type="flow") is None
    x_init = torch.randn(1, 4, 8, 8)
    got = _last_nonzero_latent(K.sample_reprise, model, x_init, sigmas,
                               restart_lo=0.90, restart_hi=0.95,
                               model_type="flow", shift=3.0,
                               generator=torch.Generator().manual_seed(7))
    assert torch.equal(got, _cogent3_pump(model, x_init, sigmas, 7))


def test_reprise_band_absent_is_cogent3_pump():
    # The img2img/low-strength case: a sliced schedule that starts at or below
    # restart_lo can host no restart, so the run is plain cogent3_pump.
    torch.manual_seed(2)
    target = torch.randn(1, 4, 8, 8)
    model = _anneal_sigma_dependent_model(target)
    full = S.flow_matching_schedule(16, shift=3.0)
    sigmas = full[[i for i in range(len(full)) if float(full[i]) <= 0.3]]
    assert float(sigmas[0]) <= 0.3 and float(sigmas[-1]) == 0.0
    x_init = torch.randn(1, 4, 8, 8)
    got = _last_nonzero_latent(K.sample_reprise, model, x_init, sigmas,
                               model_type="flow", shift=3.0,
                               generator=torch.Generator().manual_seed(9))
    assert torch.equal(got, _cogent3_pump(model, x_init, sigmas, 9))


def test_reprise_constant_x0_ends_clean():
    # Whatever the jumps do in the middle, the final segment lands on the x0
    # estimate at σ=0 like every data-prediction sampler here.
    target = torch.full((1, 16, 4, 4), 0.1)
    sigmas = S.flow_matching_schedule(16, shift=3.0)
    x_init = torch.randn(1, 16, 4, 4)
    out = K.sample_reprise(const_denoiser(target), x_init, sigmas,
                           model_type="flow", shift=3.0,
                           generator=torch.Generator().manual_seed(0))
    assert torch.isfinite(out).all()
    assert torch.allclose(out, target, atol=1e-4)
...
def test_reprise_seed_reproducible_and_stochastic():
    # Same seed ⇒ bit-identical, different seed ⇒ a different sample.
    torch.manual_seed(3)
    target = torch.randn(1, 4, 8, 8)
    model = _anneal_sigma_dependent_model(target)
    sigmas = S.flow_matching_schedule(16, shift=3.0)
    ...
    def run(seed):
        return _last_nonzero_latent(K.sample_reprise, model, x_init, sigmas,
                                    model_type="flow", shift=3.0,
                                    generator=torch.Generator().manual_seed(seed))

    assert torch.equal(run(4), run(4))
    assert not torch.allclose(run(4), run(5), atol=1e-5)


@pytest.mark.parametrize("sched", ["flow", "beta_mix", "smoothstep"])
@pytest.mark.parametrize("steps", [8, 16, 20, 21, 24, 32])
def test_reprise_nfe_matches_model_calls_and_callbacks(sched, steps):
    # The pipelines size the progress bar with reprise_nfe, so it has to be the
    # exact model-call count — and the callbacks it re-indexes must be a single
    # contiguous run across the segments, not restart once per segment.
    sigmas = _reprise_sigmas(sched, steps)
    target = torch.zeros(1, 4, 4, 4)
    calls = []
    seen = []
    ...
    K.sample_reprise(model, torch.randn(1, 4, 4, 4), sigmas,
                     callback=lambda i, s, x, d: seen.append(i),
                     model_type="flow", shift=3.0,
                     generator=torch.Generator().manual_seed(0))
    nfe = K.reprise_nfe(sigmas, model_type="flow")
    assert len(calls) == nfe
    assert seen == list(range(nfe))


def test_reprise_nfe_closed_form_and_tabulated_values():
    # NFE = n + K·⌈band/s⌉: cogent3 evaluates once per σ interval, so each restart
    # pays exactly its drafted band.
    for sched, steps, want in [("beta_mix", 20, 29), ("beta_mix", 28, 43),
                               ("smoothstep", 28, 40), ("flow", 22, 37)]:
        sigmas = _reprise_sigmas(sched, steps)
        assert K.reprise_nfe(sigmas, model_type="flow") == want, (sched, steps)
        i_hi, i_lo = K._reprise_indices(sigmas, 0.45, 0.85, "flow")
        band = i_lo - i_hi
        n = len(sigmas) - 1
        assert want == n + 3 * -(-band // 2)
    # no restart possible ⇒ the plain cogent3_pump count
    short = S.flow_matching_schedule(16, shift=3.0)
    sliced = short[[i for i in range(len(short)) if float(short[i]) <= 0.3]]
    assert K.reprise_nfe(sliced, model_type="flow") == len(sliced) - 1


def test_reprise_segments_geometry():
    # The worked example of docs/reprise.md §2.4: beta_mix N=21 snaps to
    # i_hi=8 (σ 0.812), i_lo=14 (σ 0.408), a 6-interval band.
    sigmas = _reprise_sigmas("beta_mix", 21)
    i_hi, i_lo = K._reprise_indices(sigmas, 0.45, 0.85, "flow")
    assert (i_hi, i_lo) == (8, 14)
    # ...and they are the FIRST indices at or below each threshold.
    assert float(sigmas[i_hi]) <= 0.85 < float(sigmas[i_hi - 1])
    assert float(sigmas[i_lo]) <= 0.45 < float(sigmas[i_lo - 1])

    segs = K._reprise_segments(sigmas, restart_lo=0.45, restart_hi=0.85,
                               restarts=3, draft_stride=2, model_type="flow")
    assert len(segs) == 4                                  # seg_0 + 2 drafts + final
    assert torch.equal(segs[0][:i_hi], sigmas[:i_hi])      # untouched high-σ approach
    assert torch.equal(segs[0][0], sigmas[0])
    assert torch.equal(segs[0][-1], sigmas[i_lo])
    for mid in segs[1:-1]:                                 # strided band, both edges
        assert torch.equal(mid, sigmas[[i_hi, i_hi + 2, i_hi + 4, i_lo]])
    assert torch.equal(segs[-1], sigmas[i_hi:])            # final pass runs to 0
    # every restart segment re-enters the band at its top
    for seg in segs[1:]:
        assert torch.equal(seg[0], sigmas[i_hi])


def test_reprise_strided_keeps_both_endpoints():
    band = torch.linspace(0.85, 0.45, 8)
    for stride in (1, 2, 3, 4, 99):
        sub = K._reprise_strided(band, stride)
        assert torch.equal(sub[0], band[0])
        assert torch.equal(sub[-1], band[-1])
        assert len(sub) >= 2


def test_reprise_jump_preserves_forward_marginal():
    # a²σ_lo² + b² == σ_hi² exactly, with a = alpha_hi/alpha_lo (flow) / 1 (VE).
    gen = torch.Generator().manual_seed(0)
    for _ in range(8):
        lo = (torch.rand((), generator=gen, dtype=torch.float64) * 0.4 + 0.1)
        hi = lo + torch.rand((), generator=gen, dtype=torch.float64) * 0.4
        x = torch.zeros(1, 1, 1, 1, dtype=torch.float64)
        for mt, want_a in [("flow", (1 - hi) / (1 - lo)), ("ve", torch.ones((), dtype=torch.float64))]:
            a = (((1.0 - hi) / (1.0 - lo)) if mt == "flow" else torch.ones_like(hi))
            b = (hi ** 2 - (a * lo) ** 2).clamp(min=0).sqrt()
            assert abs(float(a - want_a)) < 1e-12
            assert abs(float(a ** 2 * lo ** 2 + b ** 2 - hi ** 2)) < 1e-12
            # the helper uses exactly those coefficients (zero-noise probe: x=0 ⇒
            # output is b·ε, and a fixed generator reproduces ε)
            g1, g2 = torch.Generator().manual_seed(1), torch.Generator().manual_seed(1)
            got = K._reprise_jump(x, lo, hi, mt, g1)
            assert torch.allclose(got, b * torch.randn(x.shape, generator=g2, dtype=x.dtype))

    # statistical check: a latent on the forward marginal at σ_lo lands on the one
    # at σ_hi given the same x0.
    torch.manual_seed(0)
    lo, hi = torch.tensor(0.408), torch.tensor(0.812)
    x0 = torch.randn(1, 4, 64, 64)
    x = (1 - lo) * x0 + lo * torch.randn_like(x0)
    out = K._reprise_jump(x, lo, hi, "flow", torch.Generator().manual_seed(0))
    resid = out - (1 - hi) * x0
    assert abs(float(resid.var()) - float(hi ** 2)) < 0.02


def test_reprise_ve_finite_and_lands_clean():
    # The core is family-agnostic even though only Anima lists the sampler.
    target = torch.full((1, 4, 4, 4), 0.2)
    sigmas = _ve_sigmas()
    x_init = torch.randn(1, 4, 4, 4) * sigmas[0]
    assert K._reprise_indices(sigmas, 0.45, 0.85, "ve") is not None
    out = K.sample_reprise(const_denoiser(target), x_init, sigmas, model_type="ve",
                           generator=torch.Generator().manual_seed(0))
    assert torch.isfinite(out).all()
    assert torch.allclose(out, target, atol=1e-3)


def test_reprise_bad_args_raise():
    sigmas = S.flow_matching_schedule(8, shift=3.0)
    x = torch.randn(1, 4, 4, 4)
    model = const_denoiser(torch.zeros(1, 4, 4, 4))
    for kw in [dict(restarts=-1), dict(draft_stride=0),
               dict(restart_lo=0.85, restart_hi=0.45), dict(restart_lo=0.0),
               dict(restart_hi=1.0)]:
        with pytest.raises(ValueError):
            K.sample_reprise(model, x, sigmas, model_type="flow", shift=3.0, **kw)


def test_reprise_final_pass_is_one_continuous_solve():
    # restarts+1 inner cogent3 solves, every one carrying the caller's knobs; only
    # the last runs the full-density band all the way to σ=0.
    sigmas = _reprise_sigmas("beta_mix", 21)
    i_hi, _ = K._reprise_indices(sigmas, 0.45, 0.85, "flow")
    seen = []
    real = K.sample_cogent3

    def spy(model, x, sigs, **kw):
        seen.append((sigs, kw))
        return real(model, x, sigs, **kw)

    K.sample_cogent3 = spy
    try:
        K.sample_reprise(const_denoiser(torch.zeros(1, 4, 4, 4)), torch.randn(1, 4, 4, 4),
                         sigmas, eta_max=0.5, gate_reduce="per_channel",
                         model_type="flow", shift=3.0,
                         generator=torch.Generator().manual_seed(0))
    finally:
        K.sample_cogent3 = real

    assert len(seen) == 4                                  # restarts + 1
    for _, kw in seen:
        assert (kw["eta_max"], kw["gate_reduce"], kw["pump_strength"]) == (0.5, "per_channel", 0.08)
    assert torch.equal(seen[-1][0], sigmas[i_hi:])
    assert float(seen[-1][0][-1]) == 0.0
    assert all(float(sigs[-1]) != 0.0 for sigs, _ in seen[:-1])


def test_reprise_registered_and_allowlisted():
    import importlib
    assert K.get_sampler("reprise") is K.sample_reprise
    mod = importlib.import_module("diffucore.pipelines._anima")
    assert "reprise" in mod._ANIMA_SAMPLERS and "reprise" in mod._FLOW_AWARE_SAMPLERS
    assert mod._NFE_OVERRIDES["reprise"] is K.reprise_nfe
